...
export class TaGraphService {
  ...
  public fetchQueryList<T>(
    ...
    return this._getWrapper({ context }).pipe(
      ...
      switchMap(() =>
        this.apollo
          .query<GraphReponseData<T[]>>(this._setupData(payload, context))
          .pipe(
            tap((data) =>
  ...
  public fetchPagedQueryList<T>(
    ...
    return this._getWrapper({ context }).pipe(
      ...
      switchMap(() =>
        this.apollo
          .query<GraphReponsePagedData<T>>(this._setupData(payload, context))
          .pipe(
            tap((data) =>
  ...
  public mutate<T>(
    ...
    return this.apollo
      .mutate<ReponseMutationData<T>>(this._setupData(payload, context))
      .pipe(
  ...
  public registerGraphEndpoint(
    graphEndpoint: GraphEndpoint,
    options?: GraphOptions
  ) {
    const url =
      options?.visitor === true && this._graphConfig?.visitor
    ...
    this.apollo.client.setLink(
      this.apollo.client.link.concat(
        ApolloLink.split(
          (operation) =>
            operation.getContext()["clientName"] === graphEndpoint.clientName,
          newHttpLink
        )
      )
    );
