...
export class FormComponent extends TaBaseComponent implements OnInit {
  ...
  public save(data: unknown) {
    const cat = this._formCategoriesService.formatFormCategory(data as CategoryFormData & { parent?: string });
  ...
  private _fetch() {
    ...
    this._categoriesService.fetchCategory$(this.id).subscribe({
      next: (category) => {
        this.form.set(this._formCategoriesService.getFormCategory(parentId, category));
      },
    });
  }
}
