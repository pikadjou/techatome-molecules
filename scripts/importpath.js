...
rl.question(
  "Entrez le numéro du partenaire que vous souhaitez choisir : ",
  (choix) => {
    ...
    if (index >= 0 && index < partenaires.length) {
      ...
      if (!fs.existsSync(tsconfigPath)) {
        console.log(
          `Le fichier tsconfig.json n'a pas été trouvé à l'emplacement : ${tsconfigPath}`
        );
        rl.close();
        return;
      }
      ...
      if (!fs.existsSync(currentTsconfigPath)) {
        console.log(
          `Le fichier tsconfig.json du répertoire courant n'a pas été trouvé : ${currentTsconfigPath}`
        );
        rl.close();
        return;
      }
      ...
      const currentTsconfig = JSON.parse(
        fs.readFileSync(currentTsconfigPath, "utf-8")
      );
      ...
      if (
        ...
        console.log(
          "La propriété paths n'existe pas dans le tsconfig.json du répertoire courant."
        );
        rl.close();
        return;
      }
      ...
      const taPaths = Object.keys(currentPaths).filter((key) =>
        key.startsWith("@ta")
      );
        ...
        (action) => {
          ...
          if (isAdd) {
            ...
            taPaths.forEach((key) => {
              ...
              console.log(
                `Chemin ajouté pour ${key}: ${tsconfig.compilerOptions.paths[key]}`
              );
            });
            console.log(
              "Les chemins @ta ont été ajoutés au tsconfig du partenaire."
            );
          } else {
            ...
            console.log(
              "Les chemins @ta ont été retirés du tsconfig du partenaire."
            );
          }
          ...
          try {
            ...
            console.log(
              `Le fichier tsconfig.json du partenaire a été mis à jour avec les changements.`
            );
          } catch (error) {
            console.error(
              "Erreur lors de l'écriture du fichier tsconfig.json:",
              error
            );
          }
