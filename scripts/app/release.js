...
function checkPackageAvailability(packageName, version) {
  try {
    ...
    return (
      ...
      packageInfo.versions.includes(version)
    );
  } catch (err) {
    return false;
  }
...
async function waitForPackageAvailability(packageName, version) {
  console.log(
    `Vérification de la disponibilité de ${packageName}@${version}...`
  );
  let isAvailable = checkPackageAvailability(packageName, version);

  while (!isAvailable) {
    console.log(
      `❌ ${packageName}@${version} n'est pas encore disponible. Nouvelle vérification dans 30 secondes...`
    );
...
function removeTechatomePathsFromTsconfig() {
  try {
    ...
    console.error(
      "❌ Erreur lors de la mise à jour de tsconfig.json:",
      err.message
    );
    process.exit(1);
  }
