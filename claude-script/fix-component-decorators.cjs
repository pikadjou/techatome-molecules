...
function fixComponentDecorator(filePath) {
      ...
      if (componentMatch) {
        ...
        if (strayImportsMatch) {
          ...
          if (!decoratorContent.includes('imports:')) {
            ...
            content = content.replace(/@Component\s*\(\s*\{[^}]*\}\s*\)/s, newDecorator);
  ...
  if (!content.includes('@Component') && content.includes('export class') && content.includes('Component')) {
    ...
    if (exportsIndex > 0) {
      ...
      const selector = 'ta-' + className.replace(/Component$/, '').replace(/([A-Z])/g, '-$1').toLowerCase().replace(/^-/, '');
