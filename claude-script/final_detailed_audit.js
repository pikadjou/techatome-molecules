...
function detailedScssAudit(basePath) {
  ...
  if (problematicPx.length > 0) {
    issues.push(
      `Hardcoded px values: ${[...new Set(problematicPx)].join(", ")}`
    );
  }
  ...
  if (problematicRem.length > 0) {
    issues.push(
      `Hardcoded rem values: ${[...new Set(problematicRem)].join(", ")}`
    );
  }
...
for (const component of componentsToCheck) {
  ...
  if (result.issues.length === 0) {
    console.log(
      `✅ **${result.component}.component** - All SCSS standards met`
    );
  } else {
