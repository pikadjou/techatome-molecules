...
function auditComponent(basePath) {
  ...
  if (report.filesExist.html) {
    ...
    if (htmlContent.includes('<h1>')) htmlProblems.push('Uses <h1> instead of <ta-title level="h1">');
    if (htmlContent.includes('<h2>')) htmlProblems.push('Uses <h2> instead of <ta-title level="h2">');
    if (htmlContent.includes('<h3>')) htmlProblems.push('Uses <h3> instead of <ta-title level="h3">');
    if (htmlContent.includes('<p>')) htmlProblems.push('Uses <p> instead of <ta-text>');
    if (htmlContent.includes('<button>')) htmlProblems.push('Uses <button> instead of <ta-button>');
  ...
  if (report.filesExist.ts) {
    ...
    const needsTitleComponent = report.issues.htmlTaComponents.some(issue => 
      issue.includes('<h1>') || issue.includes('<h2>') || issue.includes('<h3>')
    );
...
components.forEach(component => {
  ...
  console.log(`- Fichiers séparés: ${report.issues.separateFiles ? '✅' : '❌'}`);
  console.log(`- HTML avec ta-*: ${report.issues.htmlTaComponents.length === 0 ? '✅' : '❌'} ${report.issues.htmlTaComponents.join(', ')}`);
  console.log(`- SCSS avec get-var: ${report.issues.scssGetVar.length === 0 ? '✅' : '❌'} ${report.issues.scssGetVar.join(', ')}`);
  console.log(`- Imports ta-*: ${report.issues.tsImports.length === 0 ? '✅' : '❌'} ${report.issues.tsImports.join(', ')}`);
  console.log('');
});